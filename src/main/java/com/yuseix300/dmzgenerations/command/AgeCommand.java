...
public final class AgeCommand {
    ...
    private static int setAge(CommandSourceStack source, Collection<ServerPlayer> targets, double years) {
        for (ServerPlayer player : targets) {
            player.getCapability(AgeCapability.INSTANCE).ifPresent(age -> age.setAgeYears(years));
            AgeBonuses.applyAll(player); // age may change the life stage → update stage bonus
    ...
    private static int addAge(CommandSourceStack source, Collection<ServerPlayer> targets, double years) {
        for (ServerPlayer player : targets) {
            player.getCapability(AgeCapability.INSTANCE).ifPresent(age -> age.addAgeYears(years));
